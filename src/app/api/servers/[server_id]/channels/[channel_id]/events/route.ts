...
export async function GET(
  ...
  const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/servers/${server_id}/channels/${channel_id}/events${search ? `?${search}` : ""}`;
...
export async function POST(
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/servers/${server_id}/channels/${channel_id}/events`,
    {
      method: "POST",
      headers: {
